...
class BroneFERPublisher:
    def __init__(self):
        ...
        self.active_mode = 'default'   # None = standby, 'mirror' / 'conversation' = aktif
        self.running     = True
    ...
    def _load_model(self):
        if not os.path.exists(Config.MODEL_PATH):
            ...
            if os.path.exists(alt):
                Config.MODEL_PATH = alt; print(f'  ✓ Found model at {alt}')
            else:
    ...
    def _publish_gaze(self, x: int, y: int, w: int, h: int,
        ...
        # Normalize ke [-1, 1], (robot hadap pengguna)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if False: # cv2.waitKey(1) & 0xFF == ord('q'):
                self.running = False
