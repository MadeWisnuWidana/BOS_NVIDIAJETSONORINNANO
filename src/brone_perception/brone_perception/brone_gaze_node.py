...
from rclpy.qos import QoSProfile, DurabilityPolicy, QoSReliabilityPolicy, QoSDurabilityPolicy, QoSHistoryPolicy
...
class FuzzyGazeNode(Node):
    ...
    def __init__(self):
        ...
        # Jetson-specific path for YOLO
        yolo_model_path = '/home/brone/brone_vision_ws/src/brone_perception/models/yolo11s.pt'
        ...
        # --- Komunikasi ROS 2 ---
        # Subscriber /image_raw: BEST_EFFORT (sesuai camera node publisher)
        ...
        # Publisher annotated_image -> RQT (TRANSIENT_LOCAL, sesuai program asli)
        annotated_qos = QoSProfile(depth=10, durability=DurabilityPolicy.TRANSIENT_LOCAL)

        self.image_publisher = self.create_publisher(
            Image, '/gaze_model/annotated_image', annotated_qos)
        
        # Publisher head_control: RELIABLE depth=10 (WAJIB cocok dengan OP3 Manager di NUC!)
        self.head_pub = self.create_publisher(
            JointState,
            '/robotis/head_control/set_joint_states',
            10)

        # Subscriber present_joint_states: RELIABLE depth=10 (WAJIB cocok dengan OP3 Manager!)
...
# --- Removed activate_head_module since it's now handled by state_machine_node with a delay ---

    def image_callback(self, msg):
        if not hasattr(self, 'uninitialized_frames'):
            self.uninitialized_frames = 0
            
        if not self.is_initialized:
            self.uninitialized_frames += 1
            if self.uninitialized_frames > 60:
                self.get_logger().warn('Timeout joint states! Memaksa init (0,0) untuk bypass CycloneDDS drop.')
                self.is_initialized = True
            else:
                return
        ...
        yolo_results = self.yolo_model.track(image_rgb, classes=0, conf=0.5, persist=True, verbose=False, device=0)
